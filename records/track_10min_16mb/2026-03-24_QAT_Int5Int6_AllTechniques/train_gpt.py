"""
Parameter Golf Submission: QAT Mixed Int5/Int6 + All Proven Techniques
Author: AppTalentHub (tsuba)
Date: 2026-03-24

Key contribution: STE Quantization-Aware Training with mixed int5 (MLP) / int6 (attention)
precision during training. The model learns weight configurations inherently robust to
post-training quantization, reducing the quantization gap and improving final BPB.

Built on the techniques from prior submissions:
- 10 layers, 512dim, MLP 3x, GQA (8 heads, 4 KV heads)
- SmearGate + BigramHash(10240, dim=128) + Orthogonal Init
- Muon WD=0.04, AdamW, momentum 0.92->0.99
- SWA (start_frac=0.4, every=50), magnitude pruning 3%
...
class Hyperparameters:
    ...
    # QAT: enable fake quantization during training
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "q_gain,skip_weight,skip_weights,smear,bigram.scale",
    ).split(",")
    if pattern
)
...
def _ste_fake_quantize(w: Tensor, clip_range: int) -> Tensor:
    ...
    Forward: quantized weights. Backward: gradients pass through as if identity."""
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training and self._qat_clip > 0:
...
class GPT(nn.Module):
    ...
    def setup_qat(self, enabled: bool = True) -> None:
        """Configure QAT clip ranges: int5 for MLP, int6 for attention."""
        for name, module in self.named_modules():
            if isinstance(module, CastedLinear) and enabled:
                if ".mlp." in name:
                    module._qat_clip = 15  # int5
                elif ".attn." in name:
                    module._qat_clip = 31  # int6
                else:
                    module._qat_clip = 0  # no QAT for embedding/head/bigram proj
...
def main() -> None:
    ...
    log0(f"qat_enabled:{args.qat_enabled} qat_warmup_steps:{args.qat_warmup_steps}")
    ...
    while True:
        ...
        if args.qat_enabled and not qat_active and step >= args.qat_warmup_steps:
            base_model.setup_qat(enabled=True)
            qat_active = True
            log0(f"qat:enabled at step:{step} (int5 MLP, int6 attention)")
    ...
    base_model.setup_qat(enabled=False)
